...
from check_digit import xpath_expression
...
def survey_rows() -> list[dict]:
    ...
    row(type="text", name="q5_03_label_serial",
        **{"label::Hausa (ha)": "Lambar lakabin samfuri (lamba 6)",
        ...
        appearance="numbers",
        # Three layers, and only the first two are enforceable in the form.
        # See docs/label_reuse.md - a self-contained form cannot check a label
        # against every earlier submission, because it has no writable store
        # that outlives the instance.
        #   (a) format and the team's allocated range
        #   (b) not already used by another child in THIS submission
        #   (c) not the label used in the LAST saved submission on this device
        constraint="regex(., '^[0-9]{6}$') and "
                   "number(.) >= number(instance('specimen_label_allocation')/root/"
                   "item[team_code=${enum_team}]/range_start) and "
                   "number(.) <= number(instance('specimen_label_allocation')/root/"
                   "item[team_code=${enum_team}]/range_end) and "
                   "count(/data/child/s5_specimen[q5_03_label_serial = current()/.]) = 1 and "
                   # pyxform exposes the previous submission through its own
                   # ${last-saved#field} syntax, not a raw instance() call. It
                   # names the instance __last-saved and declares it only when
                   # this form is used - which is why the raw call failed.
                   "count(${last-saved#q5_03_label_serial}[. = current()/.]) = 0",
        **{"constraint_message::Hausa (ha)":
           "Lambar ba ta dace ba: ko ba ta cikin kewayon ƙungiyar ka ba, "
           "ko an riga an yi amfani da ita. Duba lakabin.",
           "constraint_message::English (en)":
           "Label rejected: it is either outside your team's allocated range, "
           "already used for another child in this household, or the label used "
           "in your previous submission. Check the label book."})

    # Generated by check_digit.py, which the test suite also exercises. The
    # form and the tests therefore cannot compute different things.
    row(type="calculate", name="check_digit_expected",
        calculation=xpath_expression("${q5_03_label_serial}"))
